...
public class FaceTracker {
  ...
  public FaceTracker(Context context) {
    ...
    kaomojiList.add(new Kaomoji(
        "\u2267\u25BD\u2266", Utils.exportResource(context, R.raw.kaomoji1)));
    kaomojiList.add(new Kaomoji(
        "\uFF1E\u03C9\uFF1C", Utils.exportResource(context, R.raw.kaomoji2)));
    kaomojiList.add(new Kaomoji(
        "\uFFE3\uFF5E\uFFE3",  Utils.exportResource(context, R.raw.kaomoji3)));
    kaomojiList.add(new Kaomoji(
        "\u2190_\u2190",  Utils.exportResource(context, R.raw.kaomoji4)));
    kaomojiList.add(new Kaomoji(
        "\u256F\uFF3E\u2570",  Utils.exportResource(context, R.raw.kaomoji5)));
  ...
  public void setToSkinColor(
      Mat image, Rect rect, double[] skinColor1, double[] skinColor2,
      double t, double maxD) {
    ...
    for (int y = (int)center.y - 1; y >= rect.y; y--) {
      if (setToSkinColorOnRow(
              image, rect, y, skinColor1, t, maxD) > threshold) {
        break;
      }
    }
    for (int y = (int)center.y; y < rect.y + rect.height; y++) {
      if (setToSkinColorOnRow(
              image, rect, y, skinColor2, t, maxD) > threshold) {
        break;
      }
    }
    ...
    toBlur.width += 2 * n;
    toBlur.height += 2 * n;
    if (toBlur.width <= 0 || toBlur.height <= 0 || toBlur.x < 0 || toBlur.y < 0) {
        return;
    }
  ...
  private double setToSkinColorOnRow(
      Mat mRgba, Rect rect, int y, double[] skinValue, double t, double maxD) {
    double skinProb = 0;
