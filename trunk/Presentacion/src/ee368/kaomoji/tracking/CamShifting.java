...
public class CamShifting
{
	...
	public void create_tracked_object(Mat mRgba,List<Rect> facearray1,CamShifting cs)
	{
		...
		update_hue_image(mRgba,facearray1,cs);
		...
		//Rect roi = new Rect((int)region[0].tl().x,(int)(region[0].tl().y),region[0].width,region[0].height);//imran 
		  ...
		  tempmask=cs.obj.mask.submat(facearray1.get(0));
		 ...
		 // List<Mat> images = Arrays.asList(cs.obj.hsv.submat(roi));
		  List<Mat> images = Arrays.asList(cs.obj.huearray.get(0).submat(facearray1.get(0)));
		  ...
		  cs.obj.prev_rect=facearray1.get(0);
	...
	public RotatedRect camshift_track_face(Mat mRgba,List<Rect> region,CamShifting cs)
	{
		
		List<Float> ranges = Arrays.asList(0f, 256f);
		//ConnectedComp components;
		update_hue_image(mRgba,region,cs);
		Imgproc.calcBackProject(cs.obj.huearray, Arrays.asList(0),cs.obj.hist,cs.obj.prob, ranges,255);
